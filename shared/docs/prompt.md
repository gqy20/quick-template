...
本文档说明如何借助 AI 助手（如 GitHub Copilot、Claude、ChatGPT 等）在 py_kit 项目基础上进行高效、规范的开发。
...
py_kit 是一个标准的 Python 项目参照模板，已配置：
- ✅ 规范的项目结构（src layout）
...
```
我正在基于 py_kit 项目开发，这是一个标准的 Python 项目模板。
项目特点：
- 使用 src layout 结构
- 所有代码、注释和日志使用中文
...
```
我需要在 py_kit 项目中添加一个新的 [功能名称] 模块。

要求：
1. 在 src/py_kit/ 下创建 [module_name].py
2. 实现以下功能：
   - [功能1描述]
   - [功能2描述]
3. 使用中文注释和文档字符串
4. 使用 py_kit.logger 记录日志（中文消息）
5. 在 tests/ 下创建 test_[module_name].py
6. 测试覆盖率要达到 95% 以上
7. 在 src/py_kit/__init__.py 中导出公共 API
8. 符合项目现有的代码风格
...
1. 创建 src/py_kit/[module].py
   - 使用中文注释
   - 完整的类型注解
   - 使用 py_kit.logger 记录日志
   - 异常处理要完善
...
要求：
1. 在 src/py_kit/api.py 中实现
2. 支持异步操作
3. 使用 pydantic 进行数据验证
4. 集成 py_kit.logger 进行请求日志记录（中文）
5. 完整的错误处理和响应
6. API 文档自动生成
7. 包含认证中间件
